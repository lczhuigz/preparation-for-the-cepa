...
```English
absolute						--adj 完全的；绝对的；确凿的；独裁的

派生:absolutely					--adv 绝对地；极其
```

```English
extreme							--adj 极度的；严重的；极端的；尽头的
								--n 极端不同的感情（或境况、行为方式等）；完全相反的事物
								
派生:extremely					--adv 极其；极端；非常
```

```English
reflect							--v 反射；反映；反省

派生：reflection					--n 反射；反映；反思
```

```English
flight							--n 飞行；飞机的航程；班机
```

```English
jet								--n 喷气式飞机；喷射流；喷射口；喷嘴
```
...
短语：a dominant gene.				显性基因
```

```English
generate						--vt 产生；发生；发（电）
```

```English
generator						--n 发电机；电力公司
```

```English
generation						--n 一代；一代人；产生（电等）
```
...
派生：generally					--adv 一般地；通常地
```

```English
generous						--adj 慷慨的；宽厚的；宽宏大量的；丰富的；充足的
```
...
短语：quarrel (with sb.) about/for/over sth.		和（某人）争论某事
```
